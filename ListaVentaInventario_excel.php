...
$filename = "Lista Venta Inventario.xls";
...
header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
	...
	if($cxf = new dbFirebird($vbd))
	{
		...
		$vsql = "select m.unidad as unidad,m.descrip as descripcion,ms.existenc as existencia,ms.precio1 as precio,t.porciva as iva,coalesce(ms.precultprov,ms.costo) as costo,m.codigo as codigo,(select sum(d.preciobase*d.canmat) from dekardex as d inner join kardex as k on(k.kardexid=d.kardexid) where k.fecasentad is not null and k.fecanulado is null and k.codcomp='FV' and d.matid=m.matid) as ventas,(select sum(d.preciobase*d.canmat) from dekardex as d inner join kardex as k on(k.kardexid=d.kardexid) where k.fecasentad is not null and k.fecanulado is null and k.codcomp='DV' and d.matid=m.matid) as devoluciones,(select count(*) from dekardex as d inner join kardex as k on(k.kardexid=d.kardexid) where k.fecasentad is not null and k.fecanulado is null and k.codcomp='FV' and d.matid=m.matid) as cantidad from material as m inner join materialsuc as ms on(m.matid=ms.matid) inner join grupmat g on (m.grupmatid=g.grupmatid) inner join tipoiva as t on(m.tipoivaid=t.tipoivaid) where  m.codigo not like '%.'  and g.grupmatid not in(select gg.grupmatid from grupmat gg where gg.codigo like '00.%') and (select sum(d.parcvta) from dekardex as d inner join kardex as k on(k.kardexid=d.kardexid) where k.fecasentad is not null and k.fecanulado is null and k.codcomp='FV' and d.matid=m.matid)>0 order by (select sum(d.parcvta) from dekardex as d inner join kardex as k on(k.kardexid=d.kardexid) where k.fecasentad is not null and k.fecanulado is null and k.codcomp='FV' and d.matid=m.matid) DESC";
		if($vc = $conect_bd_actual->consulta($vsql))
		{
			...
			<div class="table-responsive" id='iniciotabla'>
				<center>
				<h4>ABC VENTA INVENTARIO</h4>
				<center>
