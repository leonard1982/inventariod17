		...
		<form action='' method='POST'>
			...
			<td>
				...
				<div class="input-group">
					...
					<div style='margin-left:10px;'>
						<button  type="button" class="btn btn-primary" id="btnExport" onclick="exportTableToExcel('dato_productos');">Exportar Excel</button></center>
					</div>
				</div>	
			</td>
		</form>
...
function exportTableToExcel(tableID, filename = ''){
    ...
    // Specify file name
    filename = filename?filename+'.xls':'ABC_Precios.xls';
...
$('#actualizar').on('click', function(){
		...
		if(opcion) {
			...
			$.ajax({
				type: "POST",
				url: "abc_precios_ajax.php",
				data: {"cant" :cant,"reg":reg ,"grupo":grupo,"linea":linea},
				success: function(response) {
					$('#contenidoprecios').html(response);
					$('.bodyp').unblock();
				}
			});
		}
