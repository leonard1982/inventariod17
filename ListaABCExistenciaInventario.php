...
<html lang="en" dir="ltr">
  ...
  <title>ABC Existencia Inventario</title>
...
<?php
	...
	$vbd = "".$linea."";
	fCrearLogTNS($_SESSION["user"],'EL USUARIO '.$_SESSION["user"].' INGRESO EN LA OPCION (ABC EXISTENCIA INVENTARIO) DEL MENU EN LA PLATAFORMA WEB DE INVENTARIOS_AUTO',$vbd);
	if($cxf = new dbFirebird($vbd))
	{
		$vsql = "select m.descrip as descripcion,ms.existenc as existencia,ms.precio1 as precio,t.porciva as iva,coalesce(ms.precultprov,ms.costo) as costo,m.codigo as codigo from material as m inner join materialsuc as ms on(m.matid=ms.matid) inner join tipoiva as t on(m.tipoivaid=t.tipoivaid) WHERE ms.existenc>0 order by ms.existenc DESC";
		...
		$vsql = "select m.descrip as descripcion,ms.existenc as existencia,ms.precio1 as precio,t.porciva as iva,coalesce(ms.precultprov,ms.costo) as costo,m.codigo as codigo,m.unidad as unidad from material as m inner join materialsuc as ms on(m.matid=ms.matid) inner join grupmat g on (m.grupmatid=g.grupmatid) inner join tipoiva as t on(m.tipoivaid=t.tipoivaid) where  m.codigo not like '%.'  and g.grupmatid not in(select gg.grupmatid from grupmat gg where gg.codigo like '00.%') and ms.existenc>0 order by ms.existenc DESC";
		if($vc = $conect_bd_actual->consulta($vsql))
		{
			...
			<div class="table-responsive">
				<center>
				<h4>ABC EXISTENCIA INVENTARIO</h4>
				<center>
					<div class="input-group" style="justify-content: center;">
						...
						<div style="margin-left:10px;">
							<button  type="button" class="btn btn-primary" id="btnExportExistenciaInventario" >Excel</button>
						</div>
						<div style="margin-left:10px;">
			...
			<?php	
					...
					while($vr = ibase_fetch_object($vc))
					{
						...
						<tr>
							...
							<td style="text-align:right;">
								<?php echo number_format(($v_total/$v_totalcompleto)*100,3,'.',','); ?>
							</td>
						...
						?>
							<tr>
								...
								<td id='marca80' colspan="2" style="background-color:yellow;">
									<center>INVENTARIO APROX 80%</center>
								</td>
								<td style="text-align:right;">
						...
						?>
							...
							<tr>
								...
								<td  id='marca15' colspan="2" style="background-color:yellow;">
									<center>INVENTARIO APROX 15%</center>
								</td>
								<td style="text-align:right;">
						...
						?>
							<tr>
								...
								<td  id='marca5' colspan="2" style="background-color:yellow;">
									<center>INVENTARIO APROX 5%</center>
								</td>
								<td style="text-align:right;">
			...
			?>
					<tr>
							...
							<td  id='marcaTOTAL' colspan="2" style="text-align:right;">
...
function exportTableToExcel(tableID, filename = ''){
    ...
    // Specify file name
    filename = filename?filename+'.xls':'ABC Existencia Inventario.xls';
...
$("#btnExportExistenciaInventario").click(function(e){
			
			e.preventDefault();
			
			
			
			window.open("ListaABCExistenciaInventario_excel.php","ventana1","width=1200,height=600,scrollbars=NO");
