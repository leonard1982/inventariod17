...
<?php
	...
	echo "<center>";
	echo "<br>";
	echo "<h2>LISTA DE PRODUCTOS CON MOVIMIENTO EN 12 MESES Y SIN EXISTENCIA</h2>";
	echo "</center>";
		...
		<form action='' method='POST'>
			...
			<td>
				...
				<div class="input-group">
					...
					<div style='margin-left:10px;'>
						<button  type="button" class="btn btn-primary" id="btnExport">Excel</button></center>
					</div>
				</div>
			</td>
		</form>
...
function exportTableToExcel(tableID, filename = ''){
    ...
    // Specify file name
    filename = filename?filename+'.xls':'Lista Con Movimiento y Sin Existencia.xls';
...
$("#btnExport").click(function(e){
			...
			window.open("ListaConMovSinExis_excel.php?reg="+reg+"&grupo="+grupo+"&linea="+linea+" ","ventana1","width=1200,height=600,scrollbars=NO");
...
$('#actualizar').on('click', function(){
		...
		if(opcion) {
			...
			$.ajax({
				type: "POST",
				url: "ListaConMovSinExis_ajax.php",
				data: {"reg":reg ,"grupo":grupo,"linea":linea},
				success: function(response) {
					$('#contenidocmovsexis').html(response);
					$('.bodyp').unblock();
				}
			});
		}
