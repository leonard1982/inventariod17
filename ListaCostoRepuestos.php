...
<html lang="en" dir="ltr">
  ...
  <title>ABC Costo Repuestos</title>
...
<?php
	...
	$vbd = "".$linea."";
	fCrearLogTNS($_SESSION["user"],'EL USUARIO '.$_SESSION["user"].' INGRESO EN LA OPCION (ABC COSTO REPUESTOS) DEL MENU EN LA PLATAFORMA WEB DE INVENTARIOS_AUTO',$vbd);
	if($cxf = new dbFirebird($vbd))
	{
		$vsql = "select m.descrip as descripcion,ms.existenc as existencia,ms.precio1 as precio,t.porciva as iva,coalesce(ms.precultprov,ms.costo) as costo,m.codigo as codigo from material as m inner join materialsuc as ms on(m.matid=ms.matid) inner join grupmat g on (m.grupmatid=g.grupmatid) inner join tipoiva as t on(m.tipoivaid=t.tipoivaid) where  m.codigo not like '%.'  and g.grupmatid in(select gg.grupmatid from grupmat gg where gg.codigo BETWEEN '01.02.' AND '01.02.VL') and ms.existenc>0 order by (ms.precio1* ms.existenc) DESC";
...
$("#btnExport").click(function(e){
			...
			window.open("ListaCostoRepuestos_excel.php","ventana1","width=1200,height=600,scrollbars=NO");
